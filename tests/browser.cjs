...
(async()=>{
 ...
 async function login(admin=false){await page.locator('#password').fill(admin?'admin-test-123':'0012');await page.locator('#login button').click();}
 ...
 await page.goto(base);await page.locator('#password').fill('wrong');await page.evaluate(()=>document.querySelector('#login').dispatchEvent(new Event('submit',{bubbles:true,cancelable:true})));await page.getByText('密码不正确，请重试',{exact:true}).waitFor();assert.equal(await page.locator('.secret').count(),0);
 ...
 await page.reload();await waitFor('#login');await login();await waitFor('#hold');assert.equal(await page.locator('.secret').first().textContent(),'0012 3456 7890');
 await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));await waitFor('#login');assert.equal(await page.locator('.secret').count(),0);await login();await waitFor('#hold');
 await hold(3200);await waitFor('#login');assert.equal(await page.locator('.secret').count(),0);await login();await waitFor('#hold');assert.equal(await page.locator('.secret').nth(1).textContent(),'000456');
